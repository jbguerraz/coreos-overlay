...
src_configure() {
	...
	local myconf=(
		...
		# and --libdir, to fix cross build issues that happen when configuring
		# gmp. For some reason, it is not enough to set only --with-sysroot,
		# also not enough to pass --with-gmp-xxx options.
		--with-sysroot="${ESYSROOT}"
		--libdir=${ESYSROOT}/usr/lib64
	)
